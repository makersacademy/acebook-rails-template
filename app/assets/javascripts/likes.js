...
$(document).ready(function () {
  ...
  function updateLikes(event) {
    ...
    if (data.error_message) {
      ...
      likeIconEl.attr("src", "Moon-full.png");
      $(this).attr(
        "action",
        `posts/${data.post_id}/likes/${data.user_like.id}`
      );
      $('<input type="hidden" name="_method" value="delete">').prependTo(
        $(this)
      );
    } else {
      let likeCount = data.like_count;
      likeCountEl.html(`${likeCount} Like${likeCount === 1 ? "" : "s"}`);
      likeIconEl.attr("src", "empty-moon.png");
