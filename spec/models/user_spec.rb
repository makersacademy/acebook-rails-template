...
RSpec.describe User, type: :model do
  ...
  context "user wall shows posts" do
    ...
    it 'where user.id equals wall_id' do
    ...
    it 'where post was written on index by wall owner' do
      post = @user.posts.create({:message => "Meow", :wall_id => nil})
      expect(User.filter(User.all, @user)).to eq [post]
    end
